...
export default function SubmissionsPage() {
    ...
    const filteredSubmissions = submissions.filter(s => 
        s.formId?.toLowerCase().includes(filter.toLowerCase()) ||
        s.page?.pageTitle?.toLowerCase().includes(filter.toLowerCase())
    );
            ...
            <div className="space-y-6">
                {loading ? (
                    <div className="space-y-4">
                        {[1, 2, 3].map(i => <div key={i} className="h-24 rounded-3xl bg-white/5 animate-pulse border border-white/5"></div>)}
                    </div>
                                    ...
                                    <div className="flex flex-col md:flex-row justify-between gap-4">
                                        <div className="space-y-1">
                                            <div className="flex items-center gap-2">
                                                <span className="badge badge-primary font-black uppercase text-[10px] tracking-widest px-2">{sub.formId}</span>
                                                <span className="text-white/30 font-bold text-xs">on</span>
                                                <Link href={`/${sub.page?.slug}`} target="_blank" className="text-white font-black hover:text-primary transition-colors">
                                                    {sub.page?.pageTitle || "Unknown Page"}
                                                </Link>
                                            </div>
                                            <div className="text-white/30 text-[10px] uppercase font-bold tracking-widest">
                                                {new Date(sub.createdAt).toLocaleString()} • IP: {sub.userIp || "N/A"}
                                            </div>
                                        </div>
                                    </div>
