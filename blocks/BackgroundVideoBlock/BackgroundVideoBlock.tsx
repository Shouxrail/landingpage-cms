...
export const BackgroundVideoBlockComponent = ({ data }: { data: any }) => {
    ...
    return (
        <section className="relative w-full min-h-[700px] flex overflow-hidden group">
            {/* Auto-playing muted background video */}
            <video
                key={url}
                className={`absolute z-0 pointer-events-none object-cover ${videoPlacement} ${videoSize}`}
                autoPlay
                loop
                muted
                playsInline
            ...
            <div className={`relative z-20 w-full flex ${placement} p-10`}>
